...
class ProductDetails extends StatefulWidget {
  ...
  const ProductDetails({Key key, this.itemProduct, this.isAgent, this.previousScreen})
      : super(key: key);
  @override
...
class _ProductDetails extends State<ProductDetails> {
  ...
  Future<Null> _handleAddCart(ProductModel itemProduct) async {
      ...
      if (serverCode == "200") {
        ...
        if (arrCartProducts.length > 0) {
          for (int i = 0; i < arrCartProducts.length; i++) {
            if (widget.itemProduct.productId ==
  ...
  _addtocartBtnPressed() {
    ...
    if (_isAddedToCart) {
      if (_itemCount == 0) {
        _handleRemove(widget.itemProduct);
      }
      else {
        _updateCart(widget.itemProduct.productId, _itemCount);
      }
    } else {
      _handleAddCart(widget.itemProduct);
    }
  }

  _handleRemove(ProductModel item) async {
    prefs = await SharedPreferences.getInstance();
    var user_id = prefs.getString('user_id');
    ...
    var requestParam = "?";
    requestParam += "user_id=" + user_id;
    requestParam += "&product_id=" + item.productId;
...
// height: 150,
                                    child: Column(
                                      ...
                                      children: [
                                        Text(
                                          itemProduct.productDescription,
                                          style: TextStyle(
                                            color: AppColors.categoryTextColor,
                                            fontSize: 14,
                                            height: 1.5,
  ...
  DcrBtn() {
    setState(() {
      ...
      _itemCount == 0 ? quantityController.clear() :quantityController.text = "$_itemCount";
